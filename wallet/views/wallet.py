...
def filter_by_date(request, wallet_id):
    ...
    if request.method == 'POST':
        ...
        expenses = filtered_expense_queryset.values('category__name').annotate(total=Sum('amount'))
        incomes = filtered_income_queryset.values('sub_category__name').annotate(total=Sum('amount'))
            
        data_expenses_category = [ i['total'] for i in list(expenses) ]
        labels_expenses_category = [i['category__name'] for i in list(expenses)]

        data_incomes_subcategory = [ i['total'] for i in list(incomes) ]
        labels_incomes_subcategory = [i['sub_category__name'] for i in list(incomes)]
