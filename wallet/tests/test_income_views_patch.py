...
def test_income_views_get(api_client, create_user):
    ...
    patched_income = Income.objects.get(id=income.id)
    print(patched_income.amount)

    assert response_detail_patch.status_code == 200
    assert income.amount != patched_income.amount
    assert patched_income.amount == data['amount']
